package sypztep.sifu.common.component.world;
	...
	public void serverTick() {
		for (int i = treesToCut.size() - 1; i >= 0; i--) {
			...
			if (tree.logs.isEmpty()) {
				AbilityHelper.mergeItemEntities(tree.drops.stream().map(drop -> new ItemEntity(obj, tree.originalPos.getX() + 0.5, tree.originalPos.getY() + 0.5, tree.originalPos.getZ() + 0.5, drop)).collect(Collectors.toList())).forEach(obj::spawnEntity);
				treesToCut.remove(i);
			}
		}
	}
