...
import sypztep.sifu.common.init.ModWorldComponents;
...
public class LumberjackEvent implements PlayerBlockBreakEvents.Before {
	...
	public static List<BlockPos> gatherTree(List<BlockPos> tree, BlockView world, BlockPos.Mutable pos, Block original) {
		if (tree.size() < maxLumberjackBlocks) {
	...
	public static boolean isWithinHorizontalBounds(List<BlockPos> tree) {
		...
		return Math.abs(maxX - minX) < maxLumberjackHorizontalLength && Math.abs(maxZ - minZ) <maxLumberjackHorizontalLength;
	}

	public static boolean canActivate(PlayerEntity player, ItemStack stack, BlockState state) {
		return !player.isSneaking() && EnchantmentUtil.hasEnchantment(ModEnchantments.LUMBERJACK, stack) && state.isIn(BlockTags.LOGS) && player.canHarvest(state);
	}

	public static boolean isValid(List<BlockPos> tree) {
		return tree.size() > 1 && tree.size() <= maxLumberjackBlocks && isWithinHorizontalBounds(tree);
	}
